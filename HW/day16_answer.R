...
str(ds.new)
# 훈련, 학습 데이터 생성
idx <- seq( 1, nrow( ds.new ), 2 );idx #홀수값 index
x.train <- ds.new[ idx, -61 ];x.train #clss열을 제외하고 홀수 행 출력
y.train <- ds.new$Class[ idx ] ;y.train #class 열의 홀수값들 
x.test <- ds.new[ -idx, -61 ] ; x.test #clss열을 제외하고 짝수 행 출력
y.test <- ds.new$Class[ -idx ] ;y.test  #class 짝수값 들 
